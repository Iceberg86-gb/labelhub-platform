...
> 整理日期:2026-06-10(当日实测入口与健康探针均 200 OK)。
...
- **本地环境**密码均为 `demo1234`。platform_admin   密码：dev-only-pa-password
- **公网演示环境的 demo 账号密码已轮换**(closure 236 生产事故账:seed 弱口令进入生产后已通过 SQL 改密焊死),登录页上硬编码的 `demo1234` 提示在公网是误导(已列挂账)。公网演示密码由项目 Owner 持有,不写入仓库。
- `platform_admin` 由后端启动时的 `PlatformAdminSeeder` 自动引导创建,初始密码来自 `.env.prod` 的 `LABELHUB_PA_INITIAL_PASSWORD`,同样仅 Owner 持有。
-  platform_admin         wgb774670647
-  owner_demo            Demo-owner-LH2026.x
-  labeler_demo          Demo-labeler-LH2026.x
-  reviewer_demo         Demo-reviewer-LH2026.x
-  senior_reviewer_demo  Demo-senior-reviewer-LH2026.x
...
| 快速自检 | `curl -f http://127.0.0.1:8443/api/actuator/health && curl -I http://127.0.0.1:8443/` |

部署脚本为四阶段闭环,rsync exclude 列表必须锚定(如 `--exclude=/submission`)且两个脚本保持同步。

## 6. 已知限制与挂账(演示前请知悉)

1. **未备案、无 HTTPS**:入口是 HTTP + IP + 非常规端口 8443,部分企业网络/浏览器策略可能拦截;备案与证书切换在挂账中。
2. **登录页 demo1234 提示在公网是误导**(本地才有效),挂账待改文案。
