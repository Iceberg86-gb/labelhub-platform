...
- [x] D 批 rejectFloor/rejectThreshold 语义澄清(纯文档零行为):契约四处 description + Policy 回退注释 + Properties javadoc + schema.d.ts 重生成(注释级)。定性实证为双语义非冗余(floor=系统兜底默认 0.20,threshold=规则生效值,Policy L36 回退链),原"统一命名"建议误诊驳回。feature 8fd78707,零碰哨兵三空 + 4 文件清单铁证;live 豁免(零行为变更,仅限文档批不立先例);MD5 换锚 cceb44d9→a158ddfa。
- [x] C 批 LabelHubLlmInteractionField 手写 fetch 收编 apiClient(行为零变更,labeling 红线 gate 白名单单文件例外):POST /ai-review/field-assist 改走 openapi-fetch 标准链,响应去手写 cast 用 generated 类型,token 前置 UX 守卫保留(client.ts authFetch L46-48 自动注入实证,双保险);JSX/文案/loading 零碰。feature dd6907a4,哨兵四证(单文件/JSX 空/labeling 其余空/零新建)+ live 双路(成功:sessionId=19 真点击 output 写回 + aiCallId=9 入草稿后端旁证;失败:停后端 Toast 不白屏)。新 fixture taskId=30/schemaId=15/sessionId=19(含 llm_interaction 字段 assist)入库可复用;原 29/14/18 无 AI 字段不适用此场景。挂账:LabelHubFileUploadField.tsx L44 attachments 手写 fetch 暂不收编(multipart 经 generated client 有行为变化风险,等真实需求连带处理)。
- [x] B 批 AI 预审规则入口卡升级生效摘要卡(纯前端零契约):AiReviewRuleEntryCard 自取 useListAiReviewRulesQuery + isCurrent 判定三态渲染(生效中·vN/Prompt 版本/维度 chips/通过+拒绝双阈值/未配置引导/loading-error 静默回退),「辅助证据」Tag 退役(OwnerLlmSettingsPage:717 他页一处文案保留);连带修复 save/publish 两 mutation 缺失的 aiReviewRulesQueryKey invalidate(实证缺陷,发布后列表陈旧)。feature 1438f1c8,哨兵四证 + live 三刀(灵魂刀:不刷新发布 v1→v2 卡片自更新,save 草稿不误触发卡片变更佐证 isCurrent 链正确;窄窗 880px 单列实测;taskId=29 未配置态双断言)。备注:formatThreshold 假定 number,契约若统一 decimal-string 需补 coerce;live 推进 taskId=30 规则至 v2 当前生效(库内状态,fixture 留痕)。
- [x] A 批 Reviewer 状态流五节点 + 可入库/可导出产品表达(纯前端零契约零新请求):旧三节点静态 strip(sticky 栏)退役,新建 ReviewFlowStrip 迁页顶通栏,deriveFlowNodes 纯函数从既有 ledger/verdict query 派生(提交→AI 预审→初审→复核/终审→终态;AI 未启用灰显/初审打回跳过复核/approved 终态标「通过·可入库/可导出」/打回统一不分级);TrustedExportCard 增一句 approved 即入库语义(#2 并入)。feature b5957e46,哨兵四证(含 details/summary 扩展符号集,裁决链六符号 diff 零出现,洁净度超 REVIEWER-UX 纯移动锚)+ live 四刀(生死刀:初审 approve 后端 status 仍 pending、终态不亮,verdict 派生含 level 判断实证,ledger #56 旁证;全通过链 28/9 五 done;打回链 28/11 跳过+danger,ledger #57;窄窗 740px 横滚实测)。live 留痕:submission 14 增 ledger #56(approve/reviewer)、submission 11 增 #57(reject/reviewer)。
- [x] DEPLOY 批 生产部署资产 + 阿里云单机上线(infra 首批,7 文件 + 五轮增量):docker-compose.prod.yml(端口收口仅 nginx 8443,密钥零 fallback fail-loud)/nginx/env example/backup/restore/README/.gitignore。增量链:F1 restore root 化 + F2 删密钥 fallback + required:true(c3704dac→a8390925)、build context 修根(94215119)、agent Dockerfile 补 api pom(731883d5,reactor)、api Dockerfile 对称补 agent pom(06bbb155)、双 pom 补 spring-boot repackage(8d297640,api 66M/agent 30M fat-jar 铁证)、backup.sh root 化对称补齐(37e7de7c,生产重跑零告警 live)。连环五缺陷定论:既有 Dockerfile/构建链从未端到端跑通过(context→reactor×2→repackage→backup 半修)。live 清单:七容器 healthy(api 44.6s 含 Flyway 27)/smoke 双 curl/PA 登录(PA_INITIAL_PASSWORD 注入链实证)/provider 录入+activate(MASTER_KEY 加解密+密文尾号回显)/业务链:schema v1 发布→数据集 30 items→labeler 提交→初审 ledger#1→senior 复核通过(A 批五节点状态流生产首秀形态全对;B 批摘要卡未配置态生产实证;DEPLOY approved 文案在 Export 台)/备份+恢复演练(27 表往返吻合,dump 完整性仲裁)/cron 装机。**未闭环项:Trusted Export 快照物化失败——导出任务提交 toast 出现但活跃/归档快照均 0,「尚未导出」,根因未定位(任务已置 ended 态为嫌疑之一),挂账诊断,全链验收以此为唯一缺口**。生产事故账:①seed 弱口令 demo 账号入生产,SQL 改密焊死(bcrypt $2y→$2a,printf 管道写库),根因 seed 无环境隔离挂账;②审计师改密命令两连坑(bash 历史展开吃 !、容器 shell 二次展开啃 $ 段)致哈希两度写空,printf 管道终解。挂账:crypto.randomUUID 仅安全上下文(HTTP+IP 瘫 designer,隧道 localhost:9443 解锁,fieldFactory.ts:6 单点);登录页硬编码 demo1234 提示成误导;备案+HTTPS;export 快照诊断。方法论:compose config 通过≠build 通过,infra Asserts 必含真实 build;既有资产存在≠可用,先端到端跑通;密码类命令单引号+长度自检+printf 管道。封板五锚: HEAD 902a07ad / MD5 a158ddfa / migrations 27 / humanpending 236 / 树干净。DEPLOY 批以"PASS 带一项挂账"封板。
...
## 250. Designer 课题对齐启动:双调研 + textarea 类型(2026-06-07)

优先级转向(owner 裁决):全流程验证让位排队,Designer 课题对齐升为主线。起因:owner 亲手按演示脚本与施工工单搭建 preference_compare schema(任务详情「去设计」入口,17 字段含 safety_flag 联动),以第一用户身份暴露体验摩擦,对照课题 4.2 要求复核。改造排序裁决:批1 textarea 类型 → 批2 rich_text 工具栏 → 批3 图片上传增强 → 批4 Designer 交互打磨;schema 资产化(模板库/复用/导入导出)排批4 之后单独立项。

双调研入档:①schema 组件能力核实(022c2759,docs/internal/schema-component-capability-research.md)——12 种 type 契约与前端完全一致,标注要求 11 组件全有落点,限制项:无独立 textarea/image type、rich_text 无工具栏、llm_interaction 与 file_upload 预览态禁用(session 约束);②课题符合度核实(78a43887,docs/internal/designer-renderer-compliance-research.md)——核心结论:同源渲染达标,Designer 预览/Labeler 作答/历史回放共用同一 SchemaFormilyRenderer + adapter + registry,「同一份 schema 两处渲染」课题灵魂要求已满足;校验四档中必填/长度/正则达标,自定义函数为白名单制(安全设计非缺陷);真差距仅三项:多行文本、富文本工具栏、图片上传。

批1 交付:textarea 第 13 个 SchemaFieldType。分支 codex/textarea-type 单 commit 9dfec980,merge 71cd2f05(owner 亲手)。契约 enum 一行 + 前端全链(palette「多行文本」/fieldFactory/编辑器复用 TextFieldEditor/LabelHubTextareaField 组件/同源 registry 注册/校验走 text 同路)+ 后端两校验器。测试:前端 295→298,后端 705→708。owner live 验收:palette 可见、字段重建为 textarea、预览真 textarea 多行换行保留、API 读回 type/required 正确。观察项:Designer 预览无提交动作故必填拦截文案未演示,labeler 作答时补验;designer 为发布即保存无独立草稿,批4 可议。

锚变更:本批为契约变更,OpenAPI MD5 新锚 ec8761e8b1fef2ae825881cd073708dc(旧 a158ddfa 自此作废)。migrations 维持 27。

同日环境作业(随本条合账):本地 22 业务表备份后 TRUNCATE 清零(保留 users/roles/user_roles/refresh_tokens/flyway);senior_reviewer_demo 本地经注册页创建并授权 SENIOR_REVIEWER;deepseek-chat provider 由 owner 重配(2-B);演示脚本与 schema 施工工单两份执行底稿就绪(不入 repo)。挂账扩充:本机 Java 环境治理(dev-api 未挂 require-java17 钩 + JAVA17_HOME 指向 Corretto 11)。

## 251. Designer 课题对齐批2:rich_text 格式工具栏(2026-06-07)

交付:rich_text 字段新增基础格式工具栏(加粗/斜体/无序列表/有序列表/插入链接),补齐课题"富文本编辑器——长文本带格式"。分支 codex/richtext-toolbar 单 commit 8d2958c1,merge(owner 亲手)。纯前端 3 文件,零契约/零后端/零迁移(git diff main -- packages/contracts services db 为空,该空 diff 自证方式自本批起为禁触标准证据)。实现:document.execCommand,零新依赖;链接 URL 白名单 http/https,javascript: 协议拒绝(含测试);只读态不渲染工具栏(代码+测试)。Semi ButtonGroup 在 Vitest 运行时解析为 undefined,绕行为 role=toolbar 容器+小号图标 Button,如实入账。测试 298→302。

live 验收(owner,Chrome):工具栏五按钮、加粗 b 标签、ul/ol 列表、https 链接生效、javascript: 拒绝均过;只读态按等效证据收(本地无历史 submission 页可开)。过程发现:Codex 内嵌浏览器不支持 prompt() 致链接项首验假 FAIL——立纪律:live 验收一律 owner 本机 Chrome,不用内嵌浏览器。
