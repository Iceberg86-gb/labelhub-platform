...
const navItemHints: Record<string, string> = {
  ...
  '/platform/labor-metrics': '提交、审核与返工计数',
  '/platform/efficiency-metrics': '复用与单位成本事实',
...
function menuIconFor(path: string) {
  ...
  if (path === '/platform/cost-metrics' || path === '/platform/efficiency-metrics') return <IconStatusFlow className={navIconClassName} />;
  if (path === '/platform/labor-metrics') return <IconReviewFlow className={navIconClassName} />;
