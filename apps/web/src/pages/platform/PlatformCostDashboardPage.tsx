...
export function PlatformCostDashboardPage() {
  ...
  return (
    <section className="platform-cost-page" aria-label="Platform token cost dashboard">
      ...
      {metrics.isError ? (
        <div className="task-state-panel">
          <Empty title="Token 成本数据加载失败" description={metrics.error instanceof Error ? metrics.error.message : '请稍后重试。'} />
        ...
        <>
          {data?.empty ? (
            <div className="task-state-panel platform-cost-empty">
              <Empty title="暂无 AI 调用记录" description="当前没有 ai_calls 事实数据。这里不会把无记录渲染成统计值为 0。" />
            </div>
          ) : null}
